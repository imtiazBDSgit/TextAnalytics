...
temp.text = readLines("https://raw.githubusercontent.com/imtiazBDSgit/TextAnalytics/master/Iron%20man%20reviews.txt")  #  iron man reviews
# head(temp.text,1)
data = data.frame(id = 1:length(temp.text), text = temp.text, stringsAsFactors = F)
# dim(data)

# Read Stopwords list
stpw1 = readLines('https://raw.githubusercontent.com/imtiazBDSgit/TextAnalytics/master/stopwords.txt')      #
...
#             Sentiment Analysis                         #
#--------------------------------------------------------#
#qualitative discourse analysis package(qdap)
